...
function createServer(apiKey: string): McpServer {
  ...
  server.tool(
    "get_swap_quote",
    `Maps to POST /quote. Returns the best swap quote across all integrated DEX protocols, with router, callData, value, price impact, route summary, and gas estimate in one response. Surplus and positive slippage are returned to the user in the same transaction. Supports an excludePositions parameter that prices the swap excluding the caller's own LP position from pool state. Returns signable data only; never signs or broadcasts. To execute the returned quote, approve the router if the input token is non-native, then send a transaction with to: router, data: callData, value: value from the user's wallet (see server execution model). ⚠️ WARNING — PRICE IMPACT: The response includes a priceImpact field. Agents and clients MUST present this value to the end user and request explicit confirmation before executing the transaction. High price impact means the user will receive significantly less value than expected. Never execute a swap silently without evaluating and disclosing the price impact.`,
    {
  ...
  server.tool(
    "get_clean_quote",
    `Maps to POST /quote with excludePositions=true. Shortcut to get_swap_quote that prices the swap as if the caller's own LP position were not in the pool, for concentrated-liquidity positions in the active tick range. Use when an agent holds a significant position in the pool it is about to trade against (rebalancing, exit, treasury sizing) and needs a quote unaffected by its own liquidity. Returns the same router/callData/value execution fields as get_swap_quote; execute the same way. ⚠️ WARNING — PRICE IMPACT: The response includes a priceImpact field. Agents and clients MUST present this value to the end user and request explicit confirmation before executing the transaction. High price impact means the user will receive significantly less value than expected. Never execute a swap silently without evaluating and disclosing the price impact.`,
    {
      chainId: z.number().int().describe("EVM chain ID (e.g. 56 for BSC)"),
  ...
  server.tool(
    ...
    `Maps to POST /addliquidity/quote. Builds a single-transaction zap to enter an LP position from any input token: handles intermediate swaps, the LP mint, and price-range setup for concentrated liquidity. Supports Uniswap V3/V4, Curve, Balancer, Aerodrome. Surplus returned to the user. Requires a poolId from search_liquidity_pools. Returns router, callData, value: to execute, approve the router if the input token is non-native, then send to: router, data: callData, value: value from the user's wallet (see server execution model). ⚠️ WARNING — PRICE IMPACT: The response includes a priceImpact field reflecting the cost of the internal swaps needed to balance the deposit. Agents and clients MUST present this value to the end user and request explicit confirmation before executing the zap. High price impact means the user's deposited value will be significantly reduced. Never execute a zap silently without evaluating and disclosing the price impact.`,
    {
      chainId: z.number().int().describe("EVM chain ID"),
  ...
  server.tool(
    ...
    `Maps to POST /removeliquidity/quote. Builds a single-transaction zap to exit an LP position. REQUIRED WORKFLOW: First call list_user_lp_positions to read the position, then pass the returned fields (positionId, nftManager, dexName, liquidityKind) here along with sender, poolId, and withdrawals. The API will return an error if required fields are missing. Returns router, callData, value: execute by sending to: router, data: callData, value: value from the user's wallet (see server execution model). ⚠️ WARNING — PRICE IMPACT: The response includes a priceImpact field reflecting the cost of swapping position tokens into the desired withdrawal token(s). Agents and clients MUST present this value to the end user and request explicit confirmation before executing the zap. High price impact means the user will receive significantly less value than expected. Never execute a zap silently without evaluating and disclosing the price impact.`,
    {
      chainId: z.number().int().describe("EVM chain ID"),
