...
- **Momentum entry**: search_liquidity_pools with \`trending: true, timeframe: "6h", signal: "entry", sortBy: "signal", sortOrder: "desc"\` → returns pools whose volume is accelerating with healthy LP flow, ranked by APR. Take the top result, read its \`suggestedRangePct\` for the price range, then follow the Zap in flow. Use timeframe "6h" for sustained traction (LP-yield), "1h" for faster reaction. Note the 6h window reflects sustained traction, not the last few minutes — for instantaneous heat, cross-check the 5m window (real-time GeckoTerminal).
- **Momentum exit**: list_user_lp_positions → for each position read \`momentumSignal\`. If "exit" (dying volume or LPs leaving) or absent (pool fell out of the momentum ranking — momentum exhausted), follow the Zap out flow.
...
Use \`hookless: true\` to exclude pools with a custom hook contract. Use \`sortBy\` ("apy", "tvl", "volume1d", "volume7d", "signal") with \`sortOrder\` to control ranking — default is tvl desc. Use \`topPerVenue\` to limit to top N pools per DEX by APY. Supports filtering by protocol/DEX, tokens, pool type, stablecoin status, and free-text search, with pagination. Required upstream step before zap_into_lp_position. IMPORTANT: The response contains two ID fields — \`poolId\` (string) must be passed AS-IS to zap_into_lp_position and zap_out_of_lp_position (do NOT construct or modify it), and \`id\` (number) is used only for analyze_pool.`,
    {
      ...
      sortBy: z.enum(["apy", "tvl", "volume1d", "volume7d", "signal"]).optional().describe("Sort field (default: tvl). 'signal' (trending only) groups pools by momentum signal entry→watch→exit (sortOrder=asc reverses) and within each group by APR descending — i.e. the entry pools with the highest APR first."),
      sortOrder: z.enum(["asc", "desc"]).optional().describe("Sort direction (default: desc)"),
      signal: z.string().optional().describe("Filter by momentum signal. Comma-separated list allowed (e.g. 'entry' or 'entry,watch'). Only effective with trending=true. Combine with sortBy=signal&sortOrder=desc to get entry pools ranked by APR."),
      topPerVenue: z.number().int().optional().describe("Limit to top N pools per venue by APY"),
      trending: z.boolean().optional().describe("If true, return only currently trending pools (with momentumSignal and suggestedRangePct on each result)"),
      timeframe: z.enum(["5m", "1h", "6h", "24h"]).optional().describe("Trending ranking window (default: 5m). Only applies with trending=true. Sent to the API as trendingDuration; also selects the volume window for feeAprEstimate. Use 6h for sustained LP-yield traction, 1h for faster reaction; 5m is real-time GeckoTerminal with the signal as hourly context."),
    ...
    async ({ chainId, project, dexKind, tokens, search, poolType, hookless, stableOnly, semiStableOnly, sortBy, sortOrder, signal, topPerVenue, trending, timeframe, page, pageSize }) => {
