...
function createServer(apiKey: string): McpServer {
  ...
  server.tool(
    "get_swap_quote",
    `Maps to POST /quote. Returns the best swap quote across all integrated DEX protocols, with router, callData, value, price impact, route summary, and gas estimate in one response. Surplus and positive slippage are returned to the user in the same transaction. Supports an excludePositions parameter that prices the swap excluding the caller's own LP position from pool state. Returns signable data only; never signs or broadcasts. EXECUTION FLOW: (1) If the input token is non-native, send an ERC-20 approve to the router and WAIT for on-chain confirmation. (2) Call this tool again for a fresh quote (quotes expire). (3) Send the tx with to: router, data: callData, value: value. See server description for full details. ⚠️ PRICE IMPACT: The response includes a priceImpact field. Agents and clients MUST present this value to the end user and request explicit confirmation before executing. High price impact means the user will receive significantly less value than expected. Never execute silently.`,
    {
  ...
  server.tool(
    "get_clean_quote",
    `Maps to POST /quote with excludePositions=true. Shortcut to get_swap_quote that prices the swap as if the caller's own LP position were not in the pool, for concentrated-liquidity positions in the active tick range. Use when an agent holds a significant position in the pool it is about to trade against (rebalancing, exit, treasury sizing) and needs a quote unaffected by its own liquidity. Returns the same router/callData/value execution fields as get_swap_quote. EXECUTION FLOW: same as get_swap_quote — approve (wait for confirmation), fresh quote, then send. See server description. ⚠️ PRICE IMPACT: The response includes a priceImpact field. Agents and clients MUST present this value to the end user and request explicit confirmation before executing. Never execute silently.`,
    {
      chainId: z.number().int().describe("EVM chain ID (e.g. 56 for BSC)"),
  ...
  server.tool(
    ...
    `Maps to POST /addliquidity/quote. Builds a single-transaction zap to enter an LP position from any input token: handles intermediate swaps, the LP mint, and price-range setup for concentrated liquidity. Supports Uniswap V3/V4, Curve, Balancer, Aerodrome. Surplus returned to the user. Requires a poolId from search_liquidity_pools. EXECUTION FLOW: (1) If the deposit token is non-native, send an ERC-20 approve to the router and WAIT for on-chain confirmation. (2) Call this tool again for a fresh quote (quotes expire). (3) Send the tx with to: router, data: callData, value: value. See server description for full details. ⚠️ PRICE IMPACT: The response includes a priceImpact field reflecting the cost of the internal swaps needed to balance the deposit. Agents and clients MUST present this value to the end user and request explicit confirmation before executing. Never execute silently.`,
    {
      chainId: z.number().int().describe("EVM chain ID"),
  ...
  server.tool(
    ...
    `Maps to POST /removeliquidity/quote. Builds a single-transaction zap to exit an LP position. REQUIRED WORKFLOW: First call list_user_lp_positions to read the position, then pass the returned fields (positionId, nftManager, dexName, liquidityKind) here along with sender, poolId, and withdrawals. The API will return an error if required fields are missing. EXECUTION FLOW: (1) For NFT-based positions (Uniswap V3/V4, PancakeSwap V3, SushiSwap V3, Algebra, Aerodrome Slipstream), approve the NFT to the router via the nftManager's approve(router, positionId) and WAIT for on-chain confirmation. For classic pools, approve the LP token to the router. (2) Call this tool again for a fresh quote (quotes expire). (3) Send the tx with to: router, data: callData, value: value. See server description for full details. ⚠️ PRICE IMPACT: The response includes a priceImpact field reflecting the cost of swapping position tokens into the desired withdrawal token(s). Agents and clients MUST present this value to the end user and request explicit confirmation before executing. Never execute silently.`,
    {
      chainId: z.number().int().describe("EVM chain ID"),
